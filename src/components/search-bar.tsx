...
export function SearchBar() {
  ...
  return (
    <Popover open={isPopoverOpen} onOpenChange={(open) => !open && setSearchQuery('')}>
        ...
        {filteredProducts.length > 0 ? (
          <div className="flex flex-col gap-2">
            {filteredProducts.map(product => (
              <Link href="#" key={product.id} className="flex items-center gap-4 p-2 rounded-md hover:bg-accent transition-colors" onClick={() => setSearchQuery('')}>
